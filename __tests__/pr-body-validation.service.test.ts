...
const prBodySignedOffButWithPlaceholders: string =
  `# Summary
...
const prBodyNotSignedOffAndNoPlaceholders: string =
  `# Summary
**Describe the changes:**
This changes X, Y, Z
...
var testCases = [
  ...
  {
    title: 'A PR Body without placeholders but not signed off should fail and return: ',
    expectedMessagePrefix: 'Please complete the final checklist',
    body: prBodyNotSignedOffAndNoPlaceholders,
    isPrBodyCompleteExpected: false
  },
  {
    title: 'A PR Body without placeholders that is signed off should pass and return: ',
...
testCases.forEach(function (testCase) {
  test(testCase.title + '"' + testCase.expectedMessagePrefix + '"', async () => {
...
test('Validate PR Body null', async () => {
  ...
  expect(result.message).toBe('The PR Body is empty - do you have the pull request template setup (docs -> pull_request_template.md)?');
});
